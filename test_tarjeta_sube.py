import unittest # clase que voy a usar para hacer los test, libreria de testing
...
class TestSube(unittest.TestCase):
    def setUp(self): #define las variables iniciales, no hace falta inicializarlas
        self.sube = Sube()
        self.sube.saldo = 1000 #instancia clase sube
    ...
    def test_imposible_pagar_pasaje_sin_saldo(self):
        sube = Sube()
        sube.saldo = 50
        with self.assertRaises(NoHaySaldoException): #atrapa no hay saldo exepcion
            sube.pagar_pasaje()
